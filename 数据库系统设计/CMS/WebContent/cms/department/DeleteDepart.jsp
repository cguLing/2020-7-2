...
<html>
   ...
   <base href="<%=basePath %>" />
    ...
    <script language="javascript">
		function firm() {//利用对话框返回的值 （true 或者 false）
			if (confirm("将会进行级联删除，确定删除？")) {
				return true;
			} else {
				//location.href = "DeleteEL.jsp";
				alert("正在执行删除操作。。。");
				return false;
			}
		}
  ...
  <div id="back">
   ...
   application.setAttribute("isAdmin", "1");
	...
	&nbsp;&nbsp;&nbsp;&nbsp;<h2><%=info%></h2>
	<br><br>
	<h3><a href="DepartShow">查看部门信息</a></h3>
	<%} 
	else{%>
	    ...
	    <form  action="DepartDelete" method="post">
	    <input type="text" placeholder="请输入删除的部门编号" name="depid" width="200"/>
	    </br>
	    </br>
	    <input type="submit" value="确认删除" onclick='javascript:return firm();'/>
	    </form>
	<%}%>
    </div>
    <div id="footer">
